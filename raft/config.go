...
type config struct {
	...
	bytes0    int64 // bytesTotal() at start of test
	maxIndex  int // the max Index in all servers
	maxIndex0 int // the max Index in all servers at start of test
}
...
// new apply command to server i must be same with other servers
...
func (cfg *config) checkOneLeader() int {
	for iters := 0; iters < 10; iters++ {
        // fmt.Printf("这是第%d 次执行checkOneleader的循环\n\n", iters)
		...
		for i := 0; i < cfg.n; i++ {
			if cfg.connected[i] {
                // fmt.Printf("checkOneLeader 可以正常访问cfg.connected[%d]\n\n", i)
				if term, leader := cfg.rafts[i].GetState(); leader {
                    // fmt.Printf("checkOneLeader 可以正常访问cfg.rafts[%d].GetState()\n\n", i)
					leaders[term] = append(leaders[term], i)
				}
                // fmt.Printf("checkOneLeader 可以从cfg.rafts[%d].GetState()中返回\n\n", i)
			}
		}
