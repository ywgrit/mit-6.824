            ...
            for i, entry := range args.Entries {
                if this := rf.LogAtIndex(entry.Index); this != nil {
                    if this.Term != entry.Term {
                        rf.log = rf.log[:rf.SubScript(this.Index)]
                        nextsubscript = i
                        break
                    }
                    nextsubscript = i + 1
...
func (rf *Raft) LogIsUpToDate(arg *RequestVoteArgs) bool {
    ...
    return arg.LastLogTerm > Term || (arg.LastLogTerm == Term && arg.LastLogIndex >= Index)
}
...
func (rf *Raft) StartElection() {
    ...
    for peer := range rf.peers {
        ...
        go func(peer int) {
                ...
                rf.mu.Lock()
                // fmt.Println("It's startelection's go func, with lock")
                defer rf.mu.Lock()
                if rf.currentTerm == RequestVoteArgs.Term && rf.state == Candidate { // because this function start with many goroutine, so rf maybe become leader or other server has become leader, so rf.state != Candidate or rf,term != args.Term is possible
                    if reply.VoteGranted {
                        ...
                        if grantedvote >= rf.Majority() {
                            ...
                            //     fmt.Printf("peer %d state = leader, rf.term = %d, 时间戳（毫秒）： %v\n\n\n", rf.me, term, time.Now().UnixMilli())
                            // }
...
func (rf *Raft) Applier() {
    ...
    for {
        ...
        rf.mu.Lock()
        // if !rf.NeedApplyWithOutLock() {
        //     rf.mu.Unlock()
        //     continue
        // }

        // for rf.lastAppied += 1; rf.lastAppied <= rf.commitIndex; rf.lastAppied++ {
        //     entry := rf.LogAtIndex(rf.lastAppied)
        //     if entry == nil {
        //         fmt.Printf("rf.lastAppied is beyond LogEntry's bound\n")
        //     }

        //     rf.applyCh <- ApplyMsg{ // can't lock channel, I don't know why, but it did cause deadlock
        //         CommandValid:       true,
...
func (rf *Raft) NeedApplyWithOutLock() bool {
    return rf.commitIndex > rf.lastAppied
}
...
func (rf *Raft) HandleReplyWithOutLock(peer int, args *AppendEntriesArgs, reply *AppendEntriesReply) {
    ...
    if reply.Success {
        ...
        lastLogIndex := LastLog(args.Entries).Index
        rf.matchIndex[peer] = lastLogIndex
        rf.nextIndex[peer] = lastLogIndex + 1

        //update commitIndex
        preCommitIndex := rf.commitIndex
        for i := rf.commitIndex; i <= lastLogIndex; i++ {
            count := 0
